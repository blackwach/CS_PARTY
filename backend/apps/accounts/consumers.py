...
class DialogsConsumer(AsyncJsonWebsocketConsumer):
    async def connect(self):
        user = self.scope.get('user')
        if not user or isinstance(user, AnonymousUser) or not user.is_authenticated:
            await self.close(code=4401)
            return
        self.user = user
        self.group_name = dialogs_group_name(user.id)
        await self.channel_layer.group_add(self.group_name, self.channel_name)
        await self.accept()
    ...
    async def receive_json(self, content, **kwargs):
        ...
        if event_type == 'dialogs.sync':
            await self.send_json({'type': 'dialogs.refresh', 'data': {'reason': 'sync'}})

    async def dialogs_refresh(self, event):
        await self.send_json({'type': 'dialogs.refresh', 'data': event.get('payload') or {}})
...
class ChatConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def receive_json(self, content, **kwargs):
        ...
        if event_type in {'typing.start', 'typing.stop'}:
            try:
                await self._typing_state(self.user.id, self.peer_id, event_type == 'typing.start')
    ...
    async def chat_typing(self, event):
        await self.send_json({'type': 'typing.state', 'data': event['payload']})
    ...
    @database_sync_to_async
    def _typing_state(self, sender_id: int, recipient_id: int, is_typing: bool):
        sender = User.objects.get(id=sender_id)
        recipient = User.objects.get(id=recipient_id)
        return send_direct_typing_state(sender, recipient, is_typing)
